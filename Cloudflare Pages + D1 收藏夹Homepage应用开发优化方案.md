...
3. **边缘函数**：Cloudflare Pages Functions（替代传统后端接口，在边缘节点运行，低延迟，处理登录验证、数据CRUD、权限控制，与Pages/D1无缝联动）。

4. **登录验证**：Cloudflare Access（生态原生，无需自己开发登录逻辑，支持邮箱/社交账号/密码登录，可配置登录策略，轻量且安全）。
...
#### 1. 登录验证：Cloudflare Access原生实现，零开发成本

- 无需自己编写账号密码逻辑，直接在Cloudflare控制台配置**登录策略**（支持邮箱验证、Google/GitHub社交登录、自定义密码登录）；

- 登录后通过`CF_Access_Identity`等请求头获取用户唯一`user_id`，传递给Pages Functions和D1，实现**用户数据隔离**；

- 配置**登录过期时间**（如7天免登），提升使用体验，同时支持手动退出登录。
