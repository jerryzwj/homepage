...
7. **配置 Cloudflare Access**
   - 导航到 Access，点击「应用程序」→「创建应用程序」
   - 选择「自托管」，输入应用名称（如 "Bookmark Homepage"）和域名（使用 Pages 分配的域名或你的自定义域名）
   - 点击「下一步」，进入「策略」配置页面
   - 点击「添加策略」，配置以下选项：
     - **策略名称**：输入策略名称（如 "用户登录"）
     - **动作**：选择「允许」
...
### 1. 首次登录
- 访问应用域名，会跳转到 Cloudflare Access 登录页面
- 使用配置的登录方式登录系统
- 登录后，系统会自动创建用户记录
