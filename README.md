...
- 收藏的增删改查操作
- 自动抓取网页信息（标题、描述、图标）
- 卡片显示网站真实图标（favicon），**本地缓存**（localStorage）零重复加载
- 图标一键**手动刷新**（悬停卡片显示刷新按钮，清缓存后重新抓取）
- 图标三级降级：本地缓存 → 远程图标 → 首字母兜底
- 标签管理，支持多标签
- 卡片点击在新标签页打开链接

### 4. 🎨 视觉设计
- **渐变主题 + 毛玻璃风格**：渐变背景、毛玻璃卡片/导航栏/下拉面板、渐变文字 Logo
- 亮色/黑暗模式切换，系统主题跟随
- 名称显示不换行，太长自动隐藏
- 容器一行显示5个卡片
...
│   │   ├── BookmarkIcon.vue     # 收藏图标组件（本地缓存+降级）
